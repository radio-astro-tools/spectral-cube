...
def check_ffmpeg(ffmpeg_cmd):
    ...
    if returncode not in (0,256):
        raise OSError("{ffmpeg} not found in the executable path.  Return code {code}"
                      .format(ffmpeg=ffmpeg_cmd, code=returncode))
...
def make_rgb_movie(cube, prefix, v1, v2, vmin, vmax, ffmpeg_cmd='ffmpeg'):
    ...
    os.system('{ffmpeg} -r 10 -y -i {prefix}%04d.png -c:v libx264 -pix_fmt yuv420p -vf '
