...
class TestFilters(BaseTest):
    ...
    def test_flatten(self):
        c, d = self.c, self.d
        expected = d[d > 0.5]
        np.testing.assert_array_equal(c.flattened(), expected)

    def test_flatten_weights(self):
        c, d = self.c, self.d
        expected = d[d > 0.5] ** 2
        np.testing.assert_array_equal(c.flattened(weights=d), expected)
    ...
    def test_slice(self):
        ...
        np.testing.assert_array_equal(c[0:3, 0:2, 0::2].flattened(), expected)
...
class TestNumpyMethods(BaseTest):
    ...
    def test_median(self):
        ...
        for y in range(m.shape[0]):
            for x in range(m.shape[1]):
                ...
                ray = ray[ray > 0.5]
                m[y, x] = np.median(ray)
        np.testing.assert_array_equal(self.c.median(axis=0), m)
    ...
    def test_percentile(self):
        ...
        np.testing.assert_array_equal(self.c.percentile(3, axis=0), m)
    ...
    def test_transpose(self, method):
        ...
        c2, d2 = cube_and_raw('vad.fits')
        for axis in [None, 0, 1, 2]:
            np.testing.assert_array_equal(getattr(c1, method)(axis=axis),
...
class TestMoment(BaseTest):
    ...
    def test_mom0(self):
        c = self.c
        np.testing.assert_array_equal(c.moment(0, axis=0), c.sum(axis=0))
...
class TestSlab(BaseTest):
    ...
    def test_slab(self):
        ...
        np.testing.assert_array_equal(c2._data, self.d[1:3])
